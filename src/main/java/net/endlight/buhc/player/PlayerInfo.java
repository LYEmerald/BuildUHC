package net.endlight.buhc.player;
...
import net.endlight.buhc.player.message.ScoreBoardMessage;
import net.endlight.buhc.player.team.TeamInfo;
import net.endlight.buhc.event.PlayerGameDeathEvent;
...
public class PlayerInfo {
    ...
    private ArrayList<String> getLore(boolean isWait){
        ...
        lore.add(" ");
        if(isWait){
            lore.add("玩家数: &l&b"+gameRoom.getPlayerInfos().size()+" &r/&l&b "+gameRoom.getRoomConfig().getMaxPlayerSize());
            lore.add("       ");
            lore.add("等待更多玩家加入中");
            lore.add("   ");
...
//            lore.add("游戏结束: &a"+formatTime(getGameRoom().loadTime));
            if(gameRoom.roomConfig.teamConfigs.size() > 1){
                for(TeamInfo teamInfo: gameRoom.getTeamInfos()){
                    ...
                    lore.add("&l"+ teamInfo +": &r  &l&b"+teamInfo.getLivePlayer().size()+"&b人 "+me);
                }
            }else{
            ...
            lore.add("      ");
            lore.add("&b击杀数:  "+killCount);
            lore.add("&e助攻数:  "+assists);
